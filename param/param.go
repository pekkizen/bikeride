...
type rideControl struct {
	...
	PowerAcceCoef     float64 `json:"accelerationPowerFactor"`
	PowerAcceMinPros  float64 `json:"minAccelerationPower (%)"`
	PowerDeceCoef     float64 `json:"decelerationPowerFactor"`
	SpeedDeceCoef     float64 `json:"decelerationSpeedFactor"`
}
...
type powermodel struct {
	...
	FlatSpeed float64 `json:"flatSpeed (km/h)"`
	FlatPower float64 `json:"flatPower (w)"`
	...
	MaxPedalledSpeed   float64 `json:"maxPedalledSpeed (km/h)"`
...
func (p *Parameters) setSystemDefaultValues() {
	...
	p.Filter.BackstepDist = 0
	p.Filter.Backsteps = 5
	p.Filter.LevelFactor = 0.5
	p.Filter.LevelMax = 4
	p.Filter.LevelMin = 1
	p.Filter.MaxFilteredEle = 10
...
func getAttributes() attributesMap {
	m := make(attributesMap, 100)
	m.put("weight.total", 40, 300, "kg", 0)
	m.put("windCourse", 0, 360, "degrees", -1)
	m.put("windSpeed", -10, 10, "m/s", 0)
	m.put("airDensity", 0.72, 1.5, "", -1)
	m.put("gravity", 9.780, 9.833, "", -1)
	m.put("temperature", -25, 45, "deg C", 0)
	m.put("airPressure", 950, 1085, "hPascals", -1)
	m.put("baseElevation", 0, 5000, "m", 0)
	m.put("minSegmentDist", 0, 200, "m", 0)

	m.put("flatSpeed", 10, 60, "km/h", -1)
	m.put("flatPower", 25, 750, "w", -1)

	m.put("verticalUpSpeed", 150, 2500, "m/h", -1)
	m.put("uphillPower", 75, 750, "w", -1)
	m.put("uphillPowerSpeed", 3, 25, "km/h", -1)
	m.put("uphillPowerGrade", 2, 10, "%", -1)

	m.put("downhillPower", 10, 25, "%", -1)
	m.put("downhillPowerGrade", -8, -1.5, "%", 0)
	m.put("maxPedalledSpeed", 15, 70, "km/h", 15)
	m.put("verticalDownSpeed", 500, 6000, "m/h", -1)
	m.put("brakingDist", 5, 100, "m", -1)

	m.put("tailWindPower", 50, 100, "%", 0)
	m.put("sysTailWind", -8, -2, "m/s", 0)
	m.put("sysHeadwind", 2, 8, "m/s", 0)
	m.put("headWindPower", 100, 150, "%", 0)
	m.put("downhillHeadwindPower", 50, 125, "%", 0)
	m.put("downhillTailwindPower", 1, 20, "%", 0)

	m.put("decelerationPowerFactor", 0.5, 1, "", 0)
	m.put("accelerationPowerFactor", 1, 2, "", 0)
	m.put("minAccelerationPower", 0, 200, "%", 0)
	m.put("decelerationSpeedFactor", 0, 1, "", 0)

	m.put("rollingResistanceCoef", 0.001, 0.02, "", 0)
	m.put("brakingFrictionCoef", 0.1, 0.5, "", 0)
	m.put("turnFrictionCoef", 0.05, 0.5, "", 0)
	m.put("airDragCoef CdA", 0.1, 1.5, "", -1)
	m.put("drivetrainLoss", 1, 20, "%", 0)

	m.put("maxSpeed", 20, 80, "km/h", 0)
	m.put("minSpeed", 2, 15, "km/h", 0)
	m.put("minLimitedSpeed", 5, 15, "km/h", 0)
	m.put("maxFreewheelPower", 0, 50, "w", 0)
	m.put("minFreewheelPower", -100, 0, "w", 0)
	m.put("heavyBrakingPower", -750, -100, "w", 0)
	...
	m.put("filter.levelMax", 1, 99, "m", 0)
	return m
}
...
func (p *Parameters) Check(l logger) error {
	...
	if Q.MaxPedalledSpeed <= 0 && Q.DownhillPowerGrade <= 0 {
...
func checkParamRange(p *Parameters, m attributesMap, l logger) {
	...
	m.check(Q.MaxPedalledSpeed, "maxPedalledSpeed", l)
	...
	m.check(p.PowerDeceCoef, "decelerationPowerFactor", l)
	m.check(p.PowerAcceCoef, "accelerationPowerFactor", l)
	m.check(p.PowerAcceMinPros, "minAccelerationPower", l)
	m.check(p.SpeedDeceCoef, "decelerationSpeedFactor", l)
...
func (p *Parameters) UnitConversionIn() {
	...
	q := &p.Powermodel
	q.FlatSpeed *= kmh2ms
	q.MaxPedalledSpeed *= kmh2ms
	q.UphillPowerSpeed *= kmh2ms
	q.UphillPowerGrade /= 100
	q.DownhillPowerGrade /= 100
	//Voimansiirtohäviö poistetaan heti ja palautetaan vasta tuloksiin
