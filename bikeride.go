...
func main() {
	...
	gpz, e := gpx.New(gpxfile, par.GPXuseXMLparser, par.GPXignoreErrors)
	if e != nil {
		l.Err(e)
		return
	}
	...
	if proFILE && len(os.Args) > 2 && os.Args[2] == "-prof" {
		cpuProfile(rou, gpz, cal, gen, par, l, devNull(0))
	}
	...
	w, e := resultWriter(par, "json")
	if e == nil {
		e = res.WriteJSON(par, w)
	}
	if e != nil {
