...
func headerLine(b []byte, sep byte, useCR bool) []byte {
	b = append(b, "seg"...)
	b = append(b, sep)
	b = append(b, "lat"...)
	b = append(b, sep)
...
func (r *Results) makeResultTXT(b []byte, p par) []byte {
	...
	wF := func(b []byte, s string, f float64, dec int, le string) []byte {
		b = append(b, s...)
		b = append(b, '\t')
		if f >= 0 {
			b = append(b, ' ')
		}
		b = numconv.Ftoa(b, f, dec, ' ') //[:len(b)-1]
		// b = numconv.TrimTrailingZeros(b) //??????????????
		return append(b, le...)
	}
	wI := func(b []byte, s string, f float64, le string) []byte {
		b = append(b, s...)
		b = append(b, '\t')
		if f >= 0 {
			b = append(b, ' ')
	...
	wS := func(b []byte, s string, x string, le string) []byte {
		b = append(b, s...)
		b = append(b, '\t')
		b = append(b, ' ')
		b = append(b, x...)
	...
	roadsegments := func(b []byte) []byte { // r.TrkpErrors
		b = wI(b, le+"Road segments         \t", float64(r.Segments), le)
		if r.TrkpRejected > 0 {
			b = wI(b, "\tNear points dropped   ", float64(r.TrkpRejected), le)
		}
		if r.TrkpErrors > 0 {
			b = wI(b, "\tInvalid points dropped", float64(r.TrkpErrors), le)
		}
		b = wS(b, "\tLength (m) ", " ", le)
	...
	technical := func(b []byte) []byte {
		...
		b = wF(b, "\tMean latitude (deg)                ", r.LatMean, d6, le)
		...
		switch p.AcceStepMode {
		case stepVel:
			b = wF(b, "\t  step lenght (m/s)              ", p.DeltaVel, d2, le)
		case stepTime:
			b = wF(b, "\t  step lenght (s)                ", p.DeltaTime, d2, le)
		case stepDist:
		...
		b = append(b, le...)
		if p.VelSolver == newtonRaphson || p.VelSolver == traubOst ||
		...
		b = wF(b, "\t  solver tolerance (m/s)        ", r.VelTol, d2, le)
		if r.SolverCalls > 0 {
		...
		b = wF(b, "\t  max relative error \t", r.VelErrorMax, d10, le)
		b = wF(b, "\t  errors > 0 (%)     \t", r.VelErrorPos*100, d1, le)
		if r.PowerErrors > 0 {
			b = wI(b, "\t  power error > 1 W  \t", float64(r.PowerErrors), le)
		}
		return b
	}
