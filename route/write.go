...
// WriteCSV (*Route) writes route data in CSV format to writer
func (o *Route) WriteCSV(p par, writer io.WriteCloser) error {
	if !p.RouteCSV {
		return nil
	}
	const segmentBytes = 150
...
// WriteTXT (*Results) writes results as tab separated text to writer
...
func (r *Results) WriteJSON(p par, writer io.WriteCloser) ([]byte, error) {
	if !p.ResultJSON {
		return nil, nil
	}
...
func (r *Results) makeResultTXT(b *[]byte, p par) {
	...
	environment := func(b *[]byte) {
		*b = append(*b, le+"Environment"+le...)
		...
		if p.AirDensity < 0 {
			...
			wF(b, "\tAir density (kg/m^3)", r.RhoBase, d3, "\t\tat base elevation"+le)
	...
	// Joules below are converted to Wh befor
...
func (r *Results) Display(p par, l *logerr.Logerr) {
	...
	if r.Filtered > 0 || p.Filter.MovingAvgWeight > 0 {
		l.Printf("%s %4.0f\n", "    Up GPX           ", r.EleUpGPX)
		l.Printf("%s %4.0f\n", "    Down GPX         ", r.EleDownGPX)
		l.Printf("%s %4.0f  %4.1f%s\n", "    Filtered         ", r.Filtered, r.FilteredPros, "%")
		l.Printf("%s %4.0f\n", "    Interpolated     ", r.Filtered-r.EleLevelled)
		if p.Filter.MovingAvgWeight > 0 {
			l.Printf("%s\n", "    (incl. averaged)")
		}
		// l.Printf("%s %4.0f\n", "    Averaged         ", r.EleAveraged )
	...
	l.Printf("%s %6.2f\n\n", "Energy efficiency (Wh/km)", r.JriderTotal/r.DistTotal)
}
