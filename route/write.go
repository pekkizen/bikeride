...
import (
	...
	num "github.com/pekkizen/numconv"
)
...
func (o *Route) makeRouteCSV(b *[]byte, p par) {
	...
	for i := 1; i <= o.segments; i++ {
		...
		num.Itoa8Pos(b, s.segnum, sep)
		num.Ftoa(b, s.lat, d6, sep)
		num.Ftoa(b, s.lon, d6, sep)
		num.Ftoa82(b, s.eleGPX, sep)
		num.Ftoa82(b, s.ele, sep)
		num.Ftoa82(b, s.ele-s.eleGPX, sep)
		num.Ftoa82(b, s.wind, sep)
		num.Ftoa82(b, s.grade*100, sep)
		num.Ftoa82(b, s.dist, sep)
		num.Itoa8Pos(b, radius, sep)
		num.Itoa8Pos(b, course, sep)
		num.Itoa(b, powerTarget, sep)
		num.Itoa8Pos(b, powerRider, sep)
		num.Itoa(b, powerBraking, sep)
		num.Ftoa82(b, s.vFreewheel*ms2kmh, sep)
		num.Ftoa82(b, s.vTarget*ms2kmh, sep)
		num.Ftoa82(b, s.vMax*ms2kmh, sep)
		num.Ftoa82(b, s.vEntry*ms2kmh, sep)
		num.Ftoa82(b, s.vExit*ms2kmh, sep)
		num.Ftoa82(b, s.time, sep)
		num.Ftoa(b, cumsec*s2h, d3, sep)
		num.Ftoa(b, cumdist*m2km, d3, sep)
		num.Itoa8Pos(b, s.calcPath, sep)
		if p.UseCR {
			num.Itoa8Pos(b, s.calcSteps, '\r')
			*b = append(*b, '\n')
		} else {
			num.Itoa8Pos(b, s.calcSteps, '\n')
		}
		cumdist += s.dist
		cumsec += s.time
	}
...
func (r *Results) makeResultTXT(b *[]byte, p par) {
	...
	rider := func(b *[]byte) {
		...
		*b = append(*b, " "+le...)
		...
		num.Ftoa(b, q.MaxPedaledSpeed, d1, 0)
		*b = append(*b, " km/h)"+le...)
		if p.ReportTech {
