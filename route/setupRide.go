...
func (s *segment) calcJoulesAndTimeFromTargets(o *Route) {
	...
	if s.powerTarget > 0 {
		o.JriderTarget += timeTarget * 0.95 * s.powerTarget // est. 5% less
	}
...
func (s *segment) setTargetVelAndPower(c *motion.BikeCalc, p par, power ratioGenerator) (err error) {
	...
	// This is the core idea of the system
	var ok bool
	s.powerTarget = p.Powermodel.FlatPower * power.Ratio(s.grade, s.wind)
	s.vTarget, ok = c.VelFromPower(s.powerTarget, -1) // -1 -> use motion velguess function
	if !ok {
...
func (s *segment) adjustTargetVelByMaxMinPedaled(c *motion.BikeCalc, p par) {
	...
	panic("adjustTargetVelByMaxMinPedaled: powerTarget < 0 or something else")
}
...
func (s *segment) setMaxVel(c *motion.BikeCalc, p par, next *segment) {
	...
	if q.LimitTurnSpeeds && s.radius < noLimRadius && s.grade < turnGradeLim {
		if vTurn := c.VelFromTurnRadius(s.radius); vMax > vTurn {
			vMax = vTurn
		}
	}
	s.vExitMax = 9999
	if q.LimitExitSpeeds && vMax > next.vMax {
		s.vExitMax = next.vMax
		if vEntry := c.MaxEntryVelNoWind(s.dist, s.vExitMax); vMax > vEntry {
			vMax = vEntry
		}
	}
...
func (s *segment) downhillMaxVel(c *motion.BikeCalc, p par) (v float64) {
	...
	if q.BrakingDist > 0 {
		if v = c.MaxBrakeStopVelNoWind(q.BrakingDist); v > q.MaxSpeed {
			v = q.MaxSpeed
		}
		return
	}
