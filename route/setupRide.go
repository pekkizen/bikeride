...
func (s *segment) targetVelAndPower(c *motion.BikeCalc, p par, power ratioGenerator) (err error) {
	...
	if s.grade < q.MinPedaledGrade-0.005*s.wind {
		if s.vFreewheel > q.MaxPedaledSpeed {
	...
	powerTarget := q.FlatPower * power.Ratio(s.grade, s.wind)
	...
	// if powerTarget > 0 {
	// 	powerTarget = c.PowerFromVel(s.vTarget) // exact
	// }
