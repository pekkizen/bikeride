...
func (o *Route) checkRide(p par, l *logerr.Logerr) {
	...
	for i := 1; i <= o.segments; i++ {
		...
		if s.vExit > p.Powermodel.MaxPedaledSpeed && s.powerRider > 0 && s.calcPath == 4 {
			l.SegMsg(1, n, "vExit =", round(s.vExit*3.6), " > maxPedaledSpeed & powerRider =", round(p.PowerOut*s.powerRider), ",", s.calcPath)
		}
		if s.powerRider > 0 && s.powerBraking < 0 && s.calcPath != 34 && s.calcPath != 324 {
			l.SegMsg(2, n, "powerRider =", round(p.PowerOut*s.powerRider), "& powerBraking =", round(s.powerBraking), ",", s.calcPath)
		}
		if s.powerBraking < 0 && s.vExit > s.vMax+0.025 {
			if failed++; failed < failLim {
				l.SegMsg(3, n, "Braking failed, powerBraking < 0 && vExit > vMax,", s.calcPath)
			}
		}
...
func (o *Route) checkRoad(p par, l *logerr.Logerr) {
	...
	for i := 1; i <= o.segments; i++ {
		...
		if math.Abs(s.ele-s.eleGPX) > maxELE || s.ipolations > maxIPO {
			l.SegMsg(2, n, "Elevation adjusted", math.Round(s.ele-s.eleGPX), "meters, interpolations", s.ipolations)
		}
		if math.Abs(s.grade) > maxGRADE {
			l.SegMsg(2, n, "Grade =", math.Round(100*s.grade), "%")
		}
	}
}
...
func (o *Route) checkRideSetup(p par, l *logerr.Logerr) {
	...
	for i := 1; i <= o.segments; i++ {
		...
		if s.vTarget < maxPedalledSpeed-veltol && s.vTarget < s.vMax && s.powerTarget <= 0 {
			l.SegMsg(1, n, "vTarget =", round(v), "< maxPedalledSpeed && vTarget < vMax && powerTarget =", round(s.powerTarget), ",", s.calcPath)
		}
