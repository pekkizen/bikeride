...
func (r *Results) calcRouteStats(o *Route) {
	...
	for i := 1; i <= last; i++ {
		...
		// sumGradeChange += math.Abs(grade - prevGrade)
		prevGrade = grade
		prevDist = dist
		// _ = prevDist
	}
	...
	// r.RelGradeChange = (100 * 100) * sumGradeChange / distsum
...
func (r *Results) calcMiscStats(c *motion.BikeCalc, p par) {
	...
	r.DownhillPowerSpeed, _ = c.NewtonRaphson(q.DownhillPower/100*q.FlatPower, 1e-8, 6)
...
func (r *Results) addSegment(s *segment, p par) {
	...
	if s.jouleRider > 0 {
		// s.powerRider = s.jouleRider / (s.time - s.timeBraking) // modify s here
		r.addRider(s, p)
	}
	r.addJoules(s)
	r.addDists(s, p)
