...
func (r *Results) CheckNum() uint64 {
	u := math.Float64bits(r.Time) << 51 >> 51
	u += math.Float64bits(r.DistTotal) << 51 >> 51
	return u
}
...
func (r *Results) calcRouteStats(o *Route) {
	...
	for i := 1; i <= o.segments; i++ {
		...
		relGradeSum += (prevGrade - grade) * (prevGrade - grade)
		prevGrade = grade
	}
	o.distance = distot
	o.eleDown = -eleDown
	...
	r.RelGradeSD = 100 * math.Sqrt(relGradeSum/float64(o.segments))
