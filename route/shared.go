...
// signbit returns the signbit of x as uint64. 0 for positive and 1 for negative numbers.
// math.Signbit(x) returns a bool.
func signbit(x float64) uint64 {
	return math.Float64bits(x) >> 63
}

// Sign returns the sign of x as float64.
// 1 for x >= +0 and -1 for x <= -0.
func Sign(x float64) float64 {
	const oneBits = 0x3FF0000000000000 // Bit representation of float64 1.0
	return math.Float64frombits(oneBits | math.Float64bits(x)&(1<<63))
	// MOVQ    X0, AX
	// MOVQ    $-9223372036854775808, CX
	// ANDQ    AX, CX
	// MOVQ    $4607182418800017408, AX
	// ORQ     CX, AX
	// MOVQ    AX, X0
	//
	// return math.Float64frombits(math.Float64bits(1) | math.Float64bits(x)&(1<<63))
...
Lord is a semi-solid white fat product obtained by rendering
